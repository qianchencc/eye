...
if [ -f "$HOME/.config/eye/tasks/$TASK" ]; then
    ...
    [ "$EYE_T_INTERVAL" -eq 900 ] && [ "$EYE_T_DURATION" -eq 30 ] && [ "$EYE_T_GROUP" == "work" ] && [ "$EYE_T_SOUND_START" == "bell" ] && [ "$EYE_T_MSG_START" == "Hello" ] && echo "PASS: add with flags" || { echo "FAIL: add flag verification"; exit 1; }
else
...
if [ -f "$HOME/.config/eye/tasks/$TASK_ZERO" ]; then
    ...
    [ "$EYE_T_DURATION" -eq 0 ] && echo "PASS: add with 0s duration" || { echo "FAIL: 0s duration verification"; exit 1; }
else
